import { TCElement } from "@src/util/Element";


const PageConfig: TCElement = {
    name: 'page-element',
    attrList: [
        [ 'title', '메인 페이지' ],
        [ 'list', [
    ...
    styles: [
        [ 'font-bold', 'font-bold text(center 5xl white sm:gray-800 md:pink-700)' ],

    ],
    render: (classEle) => {
        const [
            title, list
        ] = classEle.getAttrList();
        // console.log('classEle.tw:', classEle.tw("font-bold text(center 5xl white sm:gray-800 md:pink-700)"));
        return `
            <div class='font-bold'>
                ${title}
            </div>
